...
vec3 calc_point_light(PointLight light, vec3 normal, vec3 frag_pos, vec3 view_direction)
{
	...
	ambient = light.ambient * vec3(texture(texture_2D, tex_coords));
	diffuse = light.diffuse * diff * vec3(texture(texture_2D, tex_coords));
	
	specular = light.specular * spec * vec3(texture(texture_2D, tex_coords));
